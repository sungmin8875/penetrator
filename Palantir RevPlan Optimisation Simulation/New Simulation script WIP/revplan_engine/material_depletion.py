...
file "material_depletion.py" from ../Python Script Allocation Engine/. The ONLY change is the
import rewrite (transforms.api -> ._foundry_shim shim). The pure logic is byte-identical to the source so it
can be re-synced if the Palantir engine changes. Foundry decorators are no-ops here. The engine drives
compute() in-memory via the wrapper in run_simulation.py (compute_material_depletion) — its
revenue_plan_optimization_events input is the ENRICHED allocation output (must carry
final_production_units), and the three material inputs are read by celonis_io
(read_model_boms / read_material_inventories / read_planned_material_arrivals; ported 2026-07-16).
"""
import polars as pl
